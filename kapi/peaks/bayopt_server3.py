...
fh = logging.FileHandler(f"bayopt_server3.log")
...
def checkValidParams(params, res, k8sm):
    ...
    logger.info(f"Unique nodes: {uniqNodes}")
    return k8sm.all_pods_running(), uniqNodes, numAllocs
...
def evalpeaks(pname, valid, data_source, uniqNodes, params):
    logger.info(f"Sleeping 90 seconds for the hr-client to generate new files.")
    time.sleep(90)
    reward = 999999.0
    preward = 999999.0
...
if __name__ == "__main__":
    ...
    for i in range(0, 400):
        ...
        status, uniqNodes, numAllocs = checkValidParams(parameterization, res, k8sm)
        """
        if status == False:
        ...
        if float(numAllocs) > 132.0:
            ...
            ax_client.complete_trial(trial_index=trial_index, raw_data=evalpeaks(pname, status, data_source, uniqNodes, parameterization))
        ax_client.save_to_json_file("ax_client_snapshot3.json")
