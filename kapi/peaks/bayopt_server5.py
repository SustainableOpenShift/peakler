...
fh = logging.FileHandler(f"bayopt_server5.log")
...
hostname=".hand32-225866.bayopsys-pg0.wisc.cloudlab.us"

server_to_ip = {
    f"server2{hostname}" : "192.168.1.2",
    f"server3{hostname}" : "192.168.1.3",
    f"server4{hostname}" : "192.168.1.4",
    f"server5{hostname}" : "192.168.1.5",
    f"server6{hostname}" : "192.168.1.6",
    f"server7{hostname}" : "192.168.1.7"
}
...
def getTotalPwr(uniqNodes):
    ...
    for n in uniqNodes:
        ...
        nPwr = np.mean(arrn)
        logger.info(f"{n} {round(nPwr, 2)} W")
        totPwr = totPwr + nPwr
    return totPwr        
...
def evalpeaks(pname, data_source, uniqNodes, params):
    ...
    res = {
        ...
        pname+"pwr": (preward, 0.0),
    }
    logger.info(f"{params} {reward} {preward}")
    return res
...
if __name__ == "__main__":
    ...
    data_source = LogFolderDataSource(log_dir_path="/sharelogs",
                                      log_parser=log_parser)

    pname = "peaks"
    ax_client = AxClient.load_from_json_file("ax_client_snapshot5.json")
    """
    model_kwargs={
        "seed": 999,
    ...
    for s in HOTELRES_MICROSERVICES:
        ...
        d['value_type'] = "str"
        d['log_scale'] = False
        d['values'] = ['server2', 'server3', 'server4', 'server5', 'server6', 'server7']
        #k8sm.get_nodes(spec="server")
        params.append(d)
    ...
    ax_client.create_experiment(
        ...
        objectives={
            ...
            pname+"pwr": ObjectiveProperties(minimize=True)            
        },
    ...
    for i in range(0, 1000):
        ...
        ax_client.save_to_json_file("ax_client_snapshot5.json")
